...
const transactionsJs = `let currentEventId=null;let currentEventTimezone='UTC';let transactionCursor=null;
const paymentLabels={CASH:'現金',LINE_PAY:'LINE Pay',JKOPAY:'街口支付',OTHER:'其他'};
...
async function loadTransactions(eventId,reset){if(reset)transactionCursor=null;const params=transactionParams();if(transactionCursor)params.set('cursor',transactionCursor);$('transactions-more').disabled=true;try{const d=await api('/v2/reports/events/'+encodeURIComponent(eventId)+'/transactions?'+params);renderTransactions(d.transactions,reset);transactionCursor=d.nextCursor;$('transactions-more').hidden=!transactionCursor;const csvParams=transactionParams(false);$('transactions-csv').href='/v2/reports/events/'+encodeURIComponent(eventId)+'/transactions.csv'+(csvParams.size?'?'+csvParams:'')}finally{$('transactions-more').disabled=false}}
