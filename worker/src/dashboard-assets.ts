      ...
      <div id="content" hidden>
        ...
        <div class="grid">
          ...
          <section class="card wide">
            <div class="section-head"><h2>交易查詢</h2><a id="transactions-csv" class="ghost" download>匯出 CSV</a></div>
            <div class="transaction-filters">
              <label>付款方式<select id="transaction-payment"><option value="">全部</option><option value="CASH">現金</option><option value="LINE_PAY">LINE Pay</option><option value="JKOPAY">街口支付</option><option value="OTHER">其他</option></select></label>
              <label>狀態<select id="transaction-status"><option value="">全部</option><option value="ACTIVE">有效</option><option value="VOIDED">已作廢</option></select></label>
            </div>
            <div id="transactions" class="table-wrap"></div>
            <button id="transactions-more" class="ghost load-more" hidden>載入更多</button>
          </section>
        </div>
      </div>
    </section>
    <dialog id="transaction-detail">
      <div class="section-head"><h2 id="transaction-detail-title">交易明細</h2><button id="transaction-detail-close" class="ghost">關閉</button></div>
      <p id="transaction-detail-meta" class="muted"></p>
      <div id="transaction-detail-summary" class="detail-summary"></div>
      <div id="transaction-detail-lines" class="table-wrap"></div>
    </dialog>
  </main>
</body>
</html>`;
...
const transactionsCss = `.section-head{display:flex;justify-content:space-between;align-items:center;gap:12px}.section-head h2{margin:0}.transaction-filters{display:flex;flex-wrap:wrap;gap:12px;margin:14px 0}.transaction-filters label{display:grid;gap:5px;color:var(--muted);font-size:.78rem}.transaction-filters select{min-height:40px;border:1px solid var(--line);border-radius:9px;background:#fff;padding:0 10px;font:inherit}.transaction-row{cursor:pointer}.transaction-row:hover{background:var(--accent-soft)}.transaction-row:focus{outline:2px solid var(--accent);outline-offset:-2px}.load-more{display:block;margin:14px auto 0}dialog{width:min(720px,calc(100% - 28px));max-height:90vh;overflow:auto;border:1px solid var(--line);border-radius:18px;background:var(--card);color:var(--ink);padding:22px;box-shadow:0 24px 80px rgba(31,27,22,.25)}dialog::backdrop{background:rgba(31,27,22,.45)}.detail-summary{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:18px 0}.detail-summary div{border:1px solid var(--line);border-radius:10px;padding:10px}.detail-summary span{display:block;color:var(--muted);font-size:.75rem;margin-bottom:4px}@media(max-width:720px){.transaction-filters{display:grid;grid-template-columns:1fr 1fr}.detail-summary{grid-template-columns:1fr 1fr}}`;

const js = `const $=id=>document.getElementById(id);const money=new Intl.NumberFormat('zh-TW',{style:'currency',currency:'TWD',maximumFractionDigits:0});const paymentLabels={CASH:'現金',LINE_PAY:'LINE Pay',JKOPAY:'街口支付',OTHER:'其他'};
...
async function loadReport(id){$('status').hidden=false;$('content').hidden=true;try{const d=await api('/v2/reports/events/'+encodeURIComponent(id));currentEventId=id;currentEventTimezone=d.event.timezone;$('event-meta').textContent=d.event.status+' · '+d.event.timezone;render(d);await loadTransactions(id,true);$('status').hidden=true;$('content').hidden=false}catch(e){$('status').textContent=e.message}}
function render(d){$('summary').replaceChildren(metric('營收',money.format(d.summary.revenue)),metric('交易筆數',d.summary.transactionCount),metric('平均客單',money.format(d.summary.averageOrderValue)),metric('毛利',d.summary.grossProfit==null?'不完整':money.format(d.summary.grossProfit)));$('cost-warning').hidden=d.summary.costComplete;bars('trends',d.trends,x=>x.date,x=>x.revenue,money.format.bind(money));bars('hourly',d.hourly,x=>String(x.hour).padStart(2,'0')+':00',x=>x.revenue,money.format.bind(money));bars('payments',d.payments,x=>paymentLabels[x.method]||x.method,x=>x.revenue,money.format.bind(money));table('products',['商品','數量','營收','毛利'],d.products.map(x=>[x.name,x.quantity,money.format(x.revenue),x.grossProfit==null?'不完整':money.format(x.grossProfit)]));table('bundles',['套組','數量','營收'],d.bundles.map(x=>[x.name,x.quantity,money.format(x.revenue)]));table('pairs',['商品組合','共同交易'],d.coPurchases.map(x=>[x.firstName+' ＋ '+x.secondName,x.saleCount]));table('inventory',['商品','供應','售出','損壞','退回','結餘','去化率'],d.inventory.map(x=>[x.name,x.supplied,x.sold,x.damaged,x.returned,x.ending,x.sellThroughRate==null?'—':Math.round(x.sellThroughRate*100)+'%']))}
...
const transactionsJs = `let currentEventId=null;let currentEventTimezone='UTC';let transactionCursor=null;let transactionRequestSequence=0;
function transactionParams(includeLimit=true){const p=new URLSearchParams();const payment=$('transaction-payment').value;const status=$('transaction-status').value;if(payment)p.set('paymentMethod',payment);if(status)p.set('status',status);if(includeLimit)p.set('limit','50');return p}
async function loadTransactions(eventId,reset){const requestSequence=++transactionRequestSequence;if(reset)transactionCursor=null;const params=transactionParams();if(transactionCursor)params.set('cursor',transactionCursor);$('transactions-more').disabled=true;try{const d=await api('/v2/reports/events/'+encodeURIComponent(eventId)+'/transactions?'+params);if(requestSequence!==transactionRequestSequence||eventId!==currentEventId)return;renderTransactions(d.transactions,reset);transactionCursor=d.nextCursor;$('transactions-more').hidden=!transactionCursor;const csvParams=transactionParams(false);$('transactions-csv').href='/v2/reports/events/'+encodeURIComponent(eventId)+'/transactions.csv'+(csvParams.size?'?'+csvParams:'')}finally{if(requestSequence===transactionRequestSequence)$('transactions-more').disabled=false}}
function renderTransactions(rows,reset){const root=$('transactions');let body=$('transaction-rows');if(reset){root.replaceChildren();if(!rows.length){root.append(empty());return}const t=document.createElement('table');const head=document.createElement('thead');const hr=document.createElement('tr');['收據','時間','付款','狀態','品項','金額'].forEach(x=>{const th=document.createElement('th');th.textContent=x;hr.append(th)});head.append(hr);body=document.createElement('tbody');body.id='transaction-rows';t.append(head,body);root.append(t)}if(!body)return;rows.forEach(row=>{const tr=document.createElement('tr');tr.className='transaction-row';tr.tabIndex=0;tr.onclick=()=>showTransactionDetail(row.id);tr.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();showTransactionDetail(row.id)}};[row.receiptNumber,formatEventTime(row.occurredAtUtc),paymentLabels[row.paymentMethod]||row.paymentMethod,row.voided?'已作廢':'有效',row.itemCount,money.format(row.revenue)].forEach(x=>{const td=document.createElement('td');td.textContent=String(x);tr.append(td)});body.append(tr)})}
async function showTransactionDetail(saleId){const d=await api('/v2/reports/events/'+encodeURIComponent(currentEventId)+'/transactions/'+encodeURIComponent(saleId));const tx=d.transaction;$('transaction-detail-title').textContent=tx.receiptNumber;$('transaction-detail-meta').textContent=formatEventTime(tx.occurredAtUtc)+' · '+(paymentLabels[tx.paymentMethod]||tx.paymentMethod)+(tx.void?' · 已作廢':'');$('transaction-detail-summary').replaceChildren(detailValue('應收',money.format(tx.finalTotal)),detailValue('小費',money.format(tx.tipAmount)),detailValue('總收款',money.format(tx.revenue)),detailValue('折扣',money.format(tx.discountAmount)),detailValue('淨調整',money.format(tx.netAdjustment)),detailValue('裝置 ID',tx.deviceId));table('transaction-detail-lines',['品項','數量','單價','折扣','調整','小計'],tx.lines.map(x=>[x.displayName,x.quantity,money.format(x.unitPrice),money.format(x.allocatedDiscount),money.format(x.allocatedAdjustment),money.format(x.finalAmount)]));$('transaction-detail').showModal()}
function detailValue(label,value){const root=document.createElement('div');const l=document.createElement('span');l.textContent=label;const v=document.createElement('strong');v.textContent=value;root.append(l,v);return root}
function formatEventTime(value){return new Intl.DateTimeFormat('zh-TW',{timeZone:currentEventTimezone,year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hourCycle:'h23'}).format(new Date(value))}
$('transaction-payment').onchange=()=>currentEventId&&loadTransactions(currentEventId,true);$('transaction-status').onchange=()=>currentEventId&&loadTransactions(currentEventId,true);$('transactions-more').onclick=()=>currentEventId&&loadTransactions(currentEventId,false);$('transaction-detail-close').onclick=()=>$('transaction-detail').close();`;

export function dashboardAsset(path: string): Response | null {
  const assets: Record<string, [string, string]> = {
    "/dashboard": [html, "text/html; charset=utf-8"],
    "/dashboard/": [html, "text/html; charset=utf-8"],
    "/dashboard/dashboard.css": [css + transactionsCss, "text/css; charset=utf-8"],
    "/dashboard/dashboard.js": [js + transactionsJs, "text/javascript; charset=utf-8"],
  };
