...
**Version: v1.0.0** (`VERSION` · `versionName` · [Releases](https://github.com/lamb-liver/appforsale/releases/tag/v1.0.0))
...
| Feature | Notes |
|---------|--------|
| Quick checkout | Products / bundles, discounts, custom amount, cash / digital pay, tip |
...
| Sponsor / remove ads | Google Play one-time INAPP (`SponsorProductIds`) |
...
| Google Play Billing (tip / remove ads) | AdMob · UMP consent | kotlinx.collections.immutable |
...
│   ├── data/            # PosPersistence, PosStore, JSON codecs, CSV / backup I/O
│   └── ui/              # Activity, Compose, ViewModel, theme, billing / ads
│       └── pos/         # PosAppShell, main screen, checkout / dashboard sheets, export launchers
...
|-------|----------------|
| **ui** | `PosViewModel` observes `PosPersistence.snapshot`; cart / catalog / checkout delegate to coordinators; `PosAppShell` handles SAF export and `csvShareUriFlow` sharing; ads / billing under `ui/ads`, `ui/billing` |
| **domain** | Pure rules and coordinator results (`CartResult`, `CatalogPersistPlan`, `CheckoutWriteRequest`) |
| **data** | `PosPersistence` interface + `PosStore` (DataStore); atomic catalog writes via `applyCatalog`; `checkout` as an internal extension |
...
2. Copy local config:
   ...
   ```
   Set `sdk.dir` to your Android SDK. For production AdMob, add `admob.application.id` and `admob.banner.unit.id` (see template comments).
3. Build and test:
   ```bash
   ./gradlew :app:assembleDebug
   ./gradlew :app:testDebugUnitTest
   ```
4. Run the app: entry `com.lambliver.appforsale.ui.MainActivity`; `applicationId` matches `namespace`.

**Debug** builds always use Google’s official test ad units (hardcoded in `app/build.gradle.kts`). **Release** reads production IDs from `local.properties`; if unset, it falls back to test IDs (handy for CI; **set production IDs before store release**).

You are responsible for signing, Play Console (INAPP product IDs must match `SponsorProductIds.kt`), and the AdMob console.
...
| Item | Where | Notes |
|------|--------|--------|
| AdMob **production** App ID | `local.properties` → `admob.application.id` | Injected into `AndroidManifest` `com.google.android.gms.ads.APPLICATION_ID` |
| AdMob **production** banner unit | `local.properties` → `admob.banner.unit.id` | Injected into `BuildConfig.ADMOB_BANNER_AD_UNIT_ID` |
| AdMob **test** IDs | `app/build.gradle.kts` | Google public test IDs; safe to keep in the repo |
| Play Billing product IDs | `ui/billing/SponsorProductIds.kt` | e.g. `sponsor_tier_small`; must match Play Console; usually not in `local.properties` |
...
No Firebase `google-services.json`, backend API keys, or OAuth secrets in this repo.
...
| Secrets | Production AdMob IDs and signing passwords only in **gitignored** `local.properties` or local keystores |
| Release builds | Confirm merged Manifest / `BuildConfig` do not still use test AdMob publisher `3940256099942544` |
